...
LINKDAPI_REQUEST_INTERVAL = max(0.0, float(os.getenv("LINKDAPI_REQUEST_INTERVAL", "2.0")))
...
async def _get_client() -> Optional[AsyncLinkdAPI]:
    ...
    async with _CLIENT_LOCK:
        ...
        try:
            ...
            # Temporarily remove SOCKS proxy env vars to prevent LinkdAPI from using them
            # AsyncLinkdAPI doesn't accept custom client, so we must control via environment
            ...
            try:
                ...
                logger.info("LinkdAPI клиент успешно инициализирован без SOCKS прокси")
            finally:
